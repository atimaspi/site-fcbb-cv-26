            ...
            {navItems.map((item) => (
              <div key={item.id} className="relative group">
                {item.href ? (
                  <Link
                    to={item.href}
                    className={`px-4 py-3 rounded-lg font-semibold font-display transition-all duration-300 flex items-center relative ${
                      item.isActive
                        ? isScrolled 
                          ? 'text-cv-blue bg-cv-yellow/10 border-b-2 border-cv-yellow' 
                          : 'text-cv-yellow bg-white/10 border-b-2 border-cv-yellow'
                        : isScrolled 
